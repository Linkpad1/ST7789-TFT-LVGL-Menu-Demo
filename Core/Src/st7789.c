...
#ifdef USE_DMA
#include <string.h>
uint16_t DMA_MIN_SIZE = 64;
/* If you're using DMA, t hen u need a "framebuffer" to store datas to be displayed.
...
static void ST7789_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
	...
	ST7789_Select();
	uint16_t x_start = x0 + offset_x, x_end = x1 + offset_x;
	uint16_t y_start = y0 + offset_y, y_end = y1 + offset_y;
...
void ST7789_Fill_Color(uint16_t color)
{
	...
	ST7789_Select();
		...
		{
			memset(disp_buf, (color >> 8, color & 0xFF), sizeof(disp_buf));
